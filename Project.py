...
# =========================== YOUR ASSIGNMENT ENGINE (100% unchanged) ===========================
def assign_projects(people_file, projects_file):
    # Load data from the uploaded file (same workbook, two sheets)
    ...
    # --- Assignment loop (exactly your original logic) ---
    ...
    # === Build all the DataFrames (exactly like you had) ===
    if temp_assignments:
    ...
    # Utilization by Role with negative shortfall
    role_capacity = {}
    for role in role_columns:
        capable = people_df[people_df["Roles"].astype(str).str.contains(role, case=False, na=False)]
        role_capacity[role] = {"People": len(capable), "AvailableDays": int(capable["EffectiveCapacity"].sum())}

    required_per_role = projects_df[role_columns].sum(numeric_only=True).fillna(0).astype(int).to_dict()
    ...
    for role in role_columns:
        ...
        utilization_by_role.append({
            ...
            "Shortfall Days": assigned - available,        # ← negative = bottleneck
    ...
    utilization_by_role_df = pd.DataFrame(utilization_by_role).sort_values("Shortfall Days")

    # Project Gaps & Summary
    required_long = projects_df.melt(id_vars=["Project", "Priority"], value_vars=role_columns,
                                     var_name="Role", value_name="RequiredDays")
    ...
    totals = project_summary.groupby(["Project", "Priority"]).agg({
        "RequiredDays": "sum", "AssignedDays": "sum", "MissingDays": "sum"
    }).reset_index()
    totals["Role"] = "TOTAL"
    ...
    # === Create Excel in memory (no local file needed) ===
    output = BytesIO()
...
# ==================================== STREAMLIT APP ====================================
st.set_page_config(page_title="Project Resource Assigner", page_icon="🚀", layout="centered")

st.title("🚀 Project Resource Assigner")
st.markdown("""
Upload one Excel file containing **two sheets**:  
- `People` → columns: Name, Roles, CapacityDays, VacationDays  
- `Projects` → columns: Project, Priority (optional), and then one column per role with needed days  

You’ll get back a perfect assignment + full bottleneck analysis in seconds.
""")

uploaded_file = st.file_uploader("Upload your Excel file", type=["xlsx"])

if uploaded_file is not None:
    with st.spinner("Assigning people to projects (priority order + bottleneck detection)…"):
        result_excel = assign_projects(uploaded_file, uploaded_file)

    st.success("✅ All done! Download your results below")
    st.download_button(
        label="📥 Download final_assignments.xlsx (4 tabs ready)",
        data=result_excel,
        file_name="final_assignments.xlsx",
